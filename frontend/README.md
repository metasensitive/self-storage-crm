...
|------------|----------|--------|
| `VITE_API_URL` | Базовый URL бэкенда. **В dev оставьте пустым** — Vite-proxy перенаправит `/api/*` на `localhost:8000` (CORS не вовлечён). Заполните только для production-сборки | `https://api.example.com` |

## Если включён VPN и сайт не открывается

Некоторые VPN-клиенты перехватывают имя `localhost` и не пропускают его до loopback-интерфейса — результат: «Не удаётся получить доступ к сайту» при заходе на `http://localhost:5173`.

Решение: открывайте dev-сервер по **`http://127.0.0.1:5173`** (явный IP loopback). Vite сейчас сконфигурирован слушать все интерфейсы (`server.host = true`), а внутренний proxy на бэк уже ходит через `127.0.0.1:8000`, поэтому всё продолжает работать как обычно.

Альтернативные варианты:

- настройте split-tunneling в VPN-клиенте, чтобы трафик к `localhost`/`127.0.0.1` шёл вне туннеля;
- временно отключите VPN на время разработки.

## Решение проблемы «Не удалось связаться с сервером»

Если после логина или запроса восстановления пароля видите ошибку «Не удалось связаться с сервером», скорее всего проблема в одном из двух:

1. **Бэкенд не запущен** — `php artisan serve` в `../backend` не работает или слушает другой порт.
2. **CORS** — `VITE_API_URL` в `frontend/.env` указывает прямой URL бэка, и в `backend/.env` `FRONTEND_URL` не совпадает с реальным dev-фронтом. Решение: либо оставить `VITE_API_URL` пустым (тогда работает Vite-proxy без CORS), либо выставить `FRONTEND_URL=http://localhost:5173` в `backend/.env` и выполнить `php artisan config:clear`.
