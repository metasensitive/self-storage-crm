...
export default function UnitsPage() {
  ...
  const [bulkStatusValue, setBulkStatusValue] = useState<UnitStatus>('free');
  ...
  const bulkStatusMut = useMutation({
    mutationFn: ({ ids, status }: { ids: number[]; status: UnitStatus }) =>
      unitsApi.bulkUpdateStatus(ids, status),
    onSuccess: (res) => {
  ...
  const bulkDeleteMut = useMutation({
    mutationFn: (ids: number[]) => unitsApi.bulkRemove(ids),
    onSuccess: (res) => {
  ...
  return (
    <>
        ...
        {mode === 'grid' && containerFilter === 'all' ? (
          ...
          <>
            {isAdmin && (
              <BulkBar
                count={sel.count}
                noun={['кладовка', 'кладовки', 'кладовок']}
                onClear={sel.clear}
                actions={
                  <>
                    <Button
                ...
                <thead>
                  <tr>
                    {isAdmin && (
                      <th style={{ width: 40 }}>
                        <CheckboxTri
                          state={sel.pageState(filtered.map((u) => u.id))}
                          onChange={() => sel.togglePage(filtered.map((u) => u.id))}
      ...
      >
        <p className="t-body">
          Выберите новый статус для выделенных кладовок. Активные аренды при этом не
          проверяются — действие применится напрямую.
        </p>
        <div className="mt-3">
          <Select
      ...
      <Modal
        ...
        title={`Удалить кладовки: ${sel.count}`}
        width={460}
      >
        <p className="t-body">
          Будут удалены <strong>{sel.count}</strong> кладовок. Кладовки с активной арендой
          будут пропущены — после операции вы увидите, какие именно.
        </p>
        <div className="row gap-2 mt-4" style={{ justifyContent: 'flex-end' }}>
